 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const AS="storage";function SL(t,e,n){return t=We(t),mL(t,e,n)}function TL(t){return t=We(t),gL(t)}function Cv(t,e){return t=We(t),EL(t,e)}function IL(t=up(),e){t=We(t);const r=Gu(t,AS).getImmediate({identifier:e}),i=c1("storage");return i&&kL(r,...i),r}function kL(t,e,n,r={}){_L(t,e,n,r)}function CL(t,{instanceIdentifier:e}){const n=t.getProvider("app").getImmediate(),r=t.getProvider("auth-internal"),i=t.getProvider("app-check-internal");return new ym(n,r,i,e,pi)}function RL(){si(new Cr(AS,CL,"PUBLIC").setMultipleInstances(!0)),gn(Iv,kv,""),gn(Iv,kv,"esm2017")}RL();const AL={apiKey:"AIzaSyBd5LNvH6iIS8EzYdKCrOmXfL6FjJA07bc",authDomain:"finalproject-10d35.firebaseapp.com",projectId:"finalproject-10d35",storageBucket:"finalproject-10d35.appspot.com",messagingSenderId:"1017015914216",appId:"1:1017015914216:web:4fef4ee2e3ec2dcb8a035d"},vm=m1(AL),ku=nD(vm),Rv=Qx(vm),Av=IL(vm),NL=()=>{const[t,e]=D.useState(!1),[n,r]=D.useState(null),[i,s]=D.useState(!0),o=Yk();return D.useEffect(()=>{const a=jA(ku,l=>{s(!1),l!==null?(r(l),e(!!l)):(r(null),e(!1))});return i||o(t?"/posts":"/login"),a},[t,i]),ie(s1.Provider,{value:n,children:i?ie("div",{children:"Loading..."}):ie(pC,{})})},DL=()=>{const[t,e]=D.useState(""),[n,r]=D.useState("");function i(){bA(ku,t,n).then(({user:o})=>{console.log(o)})}function s(){VA(ku,t,n).then(({user:o})=>{console.log(o)})}return ie("div",{className:"container",children:Jn("div",{className:"login",children:[ie("div",{children:"Login / Create Account"}),ie("input",{type:"email",value:t,onChange:o=>e(o.target.value),placeholder:"Email"}),ie("input",{type:"password",value:n,onChange:o=>r(o.target.value),placeholder:"Password"}),Jn("div",{className:"login-controls",children:[ie("button",{className:"secondary",onClick:i,children:"Create Account"}),ie("button",{onClick:s,children:"Sign In"})]})]})})};var PL="AIzaSyAc9zFpuwRk68scBCEq2Y1B1yumE19sgGE";async function xL(t,e){var n="https://maps.googleapis.com/maps/api/geocode/json?latlng="+t+","+e+"&key="+PL,r=await fetch(n),i=await r.json(),s=i.results[0].formatted_address;console.log(s),open("https://www.google.com/maps/search/?api=1&query="+t+"%2C"+e)}const OL=()=>{const t=D.useContext(s1),[e,n]=D.useState([]);D.useEffect(()=>{const h=navigator.geolocation.watchPosition(m=>{i(m.coords.latitude),o(m.coords.longitude),l(!0)},m=>{console.log(m)},{enableHighAccuracy:!0});async function p(){(await dO(Ev(Rv,"posts"))).forEach(w=>{e.push({...w.data(),id:w.id})}),n([...e])}return p(),()=>navigator.geolocation.clearWatch(h)},[]);const[r,i]=D.useState(0),[s,o]=D.useState(0),[a,l]=D.useState(!1);function u(){const h=e.map(p=>Jn("div",{className:"post",children:[ie("h2",{children:p.title}),ie("img",{src:p.image,alt:"viewpoint"}),Jn("p",{children:["description ",p.description]}),Jn("div",{className:"location",id:"location",children:[ie("div",{id:"map"}),ie("button",{onClick:()=>{xL(Number(p.lat),Number(p.lon))},children:"Location"}),Jn("p",{children:["Lat: ",p.lat,", Lon: ",p.lon]}),Jn("p",{children:["ID: ",p.id]})]})]},p.id));return ie(Dm,{children:h.reverse()})}async function c(){if(d==null||g==""||v==""||t==null)return;const h=Cv(Av,`/images/${d.name||""}`);await SL(h,d);const m={image:await TL(Cv(Av,`images/${d.name}`)),title:g,description:v,lat:r.toString(),lon:s.toString(),author:(t==null?void 0:t.uid)||""},w=await fO(Ev(Rv,"posts"),m);e.push({...m,id:w.id}),n([...e]),console.log("Document written with ID: ",w.id)}const[d,f]=D.useState(),[g,y]=D.useState(""),[v,_]=D.useState("");return ie(Dm,{children:Jn("div",{className:"header",children:[ie("h1",{children:"HikePoster"}),a?`Lat: ${r}, Lon: ${s}`:"Loading...",ie("div",{children:"Make a Post"}),ie("textarea",{value:g,onChange:h=>y(h.target.value),placeholder:"Title"}),ie("textarea",{value:v,onChange:h=>_(h.target.value),placeholder:"Description"}),ie("input",{type:"file",onChange:h=>{var p;return f((p=h.target.files)==null?void 0:p[0])}}),ie("br",{}),ie("button",{onClick:()=>{c()},children:"Submit Post"}),ie("button",{onClick:()=>HA(ku),children:"Logout"}),ie("br",{}),ie("br",{}),ie("br",{}),ie("br",{}),ie("div",{className:"postssection",children:u()})]})})},LL=yC([{path:"/",element:ie(NL,{}),children:[{path:"/login",element:ie(DL,{})},{path:"/posts",element:ie(OL,{})}]}]);function ML(){return ie(dC,{router:LL})}Oh.createRoot(document.getElementById("root")).render(ie(ZS.StrictMode,{children:ie(ML,{})}));
